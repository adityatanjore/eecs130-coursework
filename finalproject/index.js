...
const myAPIEndpoint = 'https://adi-eecs130-project.herokuapp.com/photos';
const mySecondAPIEndpoint = 'https://adi-eecs130-project.herokuapp.com/comments';
...
const addVote = (ev) => {
  ...
  const question = questions.find(x => x._id === entry_id);
...
const generateThoughtEntryHTML = (entry) => {
    ...
    <div class="thoughtcard">
        <p class = "cardthoughtdate">${entry.date}</p>
        <p class = "cardthought">${entry.thought}</p>
        <p class = "cardthoughtid"> Thought ID #: ${entry._id}</p>
        <p>
            <a data-id="${entry._id}" href="#" class="deletethought">REMOVE THOUGHT</a>
        </p>
        <p id="demo"></p>
    </div>
    `;
...
const generateEntryHTML = (entry) => {
    ...
    <div class="card">
        ...
        <p class = "cardquestionid"> Question ID #: ${entry._id}</p>
        <button entry_id = "${entry._id}" class = formUpvote type="button"> <p> Upvote! </p> <strong> ${entry.votes} </strong>
        </button>
    </div>
    `;
};
...
const attachDeleteEventHandlers = () => {
    ...
    for (link of document.querySelectorAll('.deletethought')) {
        link.onclick = deleteThoughtEntry;
    }
...
const deleteThoughtEntry = (ev) => {
    const id = ev.target.getAttribute('data-id');
    fetch(mySecondAPIEndpoint + '/' + id, {
        method: 'DELETE'
    })
    .then(response => response.json())
    .then(data => {
